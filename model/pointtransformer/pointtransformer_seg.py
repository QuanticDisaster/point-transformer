...
class TransitionDown(nn.Module):
    ...
    def forward(self, pxo):
        ...
        if self.stride != 1:
            ...
            #logger.info(str(self.nsample) + " " + str(p.shape[0]) + " " + str(n_p.shape[0]) + " " + str(x.shape[0]) + " " + str(o.shape[0]) + " " + str(n_o.shape[0]))
            #logger.info(str(o) + " " + str(n_o))
            #logger.info("compute query and group in  TransitionDown :" + str(time.time() - start) +  " for " + str(p.shape[0]) + " and " + str(n_p.shape[0]) + " points")
            #logger.info("feat, offset, nex_offset :" + str(x.shape[0]) + ' ' + str(o.shape[0]) +  ' ' +str(n_o.shape[0]))
...
if custom:
    class TransitionDown(torch.nn.Module):
        ...
        def forward(self, x, pos, batch):
            ...
            # beware of self loop
            #start = time.time()
            id_k_neighbor = knn(pos.cpu(), pos[id_clusters].cpu(), k=self.k,
                                batch_x=batch.cpu(), batch_y=sub_batch.cpu()).cuda()
            
            """
            logger.info("compute k neighbors in  TransitionDown :" + str(time.time() - start) +  " for " + str(pos.shape[0]) + " and " + str(pos[id_clusters].shape[0]) + " points")
            start = time.time()
            
            m = pos.shape[0]
            nsample = 16
            pointops_cuda.knnquery_cuda(m, nsample, pos, pos[id_clusters], torch.IntTensor(1), torch.IntTensor(1), torch.cuda.IntTensor(m, nsample).zero_(), torch.cuda.FloatTensor(m, nsample).zero_())
...
class Net(torch.nn.Module):
    ...
    def forward(self, x, pos, batch=None):
        ...
        # first block
        x = self.mlp_input(x)
        edge_index = knn_graph(pos.cpu(), k=self.k, batch=batch.cpu()).cuda()
...
def pointtransformer_seg_repro(**kwargs):
    if custom:
        model = Net(3, 13, dim_model=[32, 64, 128, 256, 512], k=16)
    else:
